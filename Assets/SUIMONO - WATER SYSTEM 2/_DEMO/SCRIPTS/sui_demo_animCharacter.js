...
function Update () {
	...
	if (currClip != useClip){
		...
		if (animation[useClip] != null && animation[currClip] != null){
			animation[useClip].time = animation[currClip].time;
		}
		
		//set new clip
		currClip = useClip;
		animTime = 0.0;
		if (animation[currClip] != null){
			this.animation.CrossFade(currClip,fadeSpeed);
			animation[currClip].speed = playSpeed;
		} else {
