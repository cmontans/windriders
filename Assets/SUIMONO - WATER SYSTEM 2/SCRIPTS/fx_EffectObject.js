...
function Start () {
	...
	InvokeRepeating("SetUpdate",0.1,(1.0/30.0));
}
...
function AddRule(){
	...
	tempIndex  = ruleIndex;
	tempData = effectData;
	if (effectRule == null) effectRule = new Sui_FX_Rules[tempRules.Length+1];
	if (ruleIndex == null) ruleIndex = new int[tempRules.Length+1];
	if (effectData == null) effectData = new float[tempRules.Length+1];
...
function DeleteRule(ruleNum : int){
	...
	if (endLP <= 0){
		...
		if (effectRule == null) effectRule = new Sui_FX_Rules[endLP];
		if (ruleIndex == null) ruleIndex = new int[endLP];
		if (effectData == null) effectData = new float[endLP];
